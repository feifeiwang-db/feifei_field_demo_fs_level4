...
# MAGIC * Additional note for Azure users: The Cosmos DB online store uses a different schema than the offline store. To ensure that Feature Store can access the Cosmos DB online store, you must create the table in the online store by using `publish_table()`.  Do not manually create a database or container with primary key inside Cosmos DB. The publish table command does that for you automatically. 
# MAGIC 
